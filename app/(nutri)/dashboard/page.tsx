...
export default async function DashboardPage() {
  ...
  const [patientsRes, appointmentsRes, checkinsRes, financialRes, tasksRes, plansRes, profileRes] = await Promise.all([
  ...
  const scores = checkinsThisWeek.map((c) => c.diet_score).filter((s) => typeof s === "number");
  ...
  return (
    <DashboardClient
      userName={profileRes.data?.full_name || user?.email?.split("@")[0] || "Nutricionista"}
      patients={patients}
      plans={plans}
